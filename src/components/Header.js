...
const Header = () => {
  ...
  // Cottage types from your dropdown image
  const cottageTypes = [
    ...
    <header className={`header ${scrolled ? 'scrolled' : ''}`}>
      ...
      <nav className={`nav ${menuOpen ? 'open' : ''}`}>
        ...
        <div className="dropdown-container" onClick={(e) => e.stopPropagation()}>
          <button
            className={`nav-link dropdown-toggle ${activeLink === 'Cottages' ? 'active' : ''}`}
        ...
        <a href="/ownacottage"><button className="own-btn">Own a cottage</button></a>
      </nav>
    </header>
  );
};
